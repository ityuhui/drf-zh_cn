...
DRF算法的“策略防伪”属性显示，一个用户不会因为她要的多就会得到更好的分配。下列例子表明，过多的需求事实上会降低该用户的分配，从而导致主导份额较低。假设一个集群里有两种资源和10个用户，第一个用户的需求向量是 <1, 0>，其他用户的需求向量都是 <0, 1>。第一个用户获得了第一种资源的全部，其他用户每人都获得了第二种资源的 $ \frac 1 9$。如果第一个用户将其需求向量变更为 <1, 1>，她将只能分得每种资源得 $\frac 1 {10} $，其他用户分别得到第二种资源的 $\frac 1 {10} $。
