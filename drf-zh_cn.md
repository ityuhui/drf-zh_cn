...
加权的“最大-最小公平”算法的吸引力在于，它很通用，可以支持多种其他的资源分配算法，如基于优先级、资源预留、以及截止日期【31】。它还确保隔离性，即不管其他用户的需求如何，一个用户一定会收到属于它的份额。
...
#### 证明
举一个反例：假设用户 $i$ 没有任何一种资源被用尽，但是这与 DRF 渐进式填充算法相冲突。如果当用户 $i$ 没有任意一种用尽的资源，渐进式填充算法就会继续为其分配资源（其他用户只共享非饱和资源）。

渐进式填充算法总是根据用户的需求向量，按比例为其分配资源。更准确的说，设 $D_i=<d_{i,1}, d_{i,2}, ..., d_{i,m}>$是用户 $i$ 的需求向量，那么，在渐进式填充算法的任何时刻 $t$，用户 $i$ 的分配都与其需求向量成正比，
$$ A_i(t) = \alpha_i(t) \cdot D_i = \alpha_i(t) \cdot <d_{i,1}, d_{i,2}, ..., d_{i,m}>     (1)$$ 
$\alpha_i(t)$ 是一个正标量。
现在我们来证明 DRF 算法的属性。

#### 定理9 DRF 是 Pareto 高效的

#### 证明
假设用户 $i$ 可以在不降低其他人的主导份额的前提下，提高自己的主导份额 $s_i$，根据引理8， 用户 $i$ 至少有一个饱和资源，那么，如果没有其他用户正在使用该饱和资源，那么我们不可能增加用户$i$ 的饱和资源的份额。如果其他用户正在使用这种饱和资源，那么增加用户 $i$ 的分配将降低至少一个用户 $j$ 的该资源的分配。而在渐进式填充下，任何用户的资源都是根据她的需求向量（等式1）来按比例分配的，降低用户 $i$ 的任何资源都将降低 $i$ 的主导份额。这与我们的假设相冲突，因此可以得到证明。

#### 定理10 DRF 满足“共享激励”和“瓶颈公平性”属性

#### 证明
考虑一个有 $n$ 个用户的系统。假设 $k$ 是渐进式填充的第一个饱和资源。假设用户 $i$ 得到了资源 $k$ 的最多的份额， 让 $t_{i,k}$ 表示她得到的资源 $k$ 的份额。因为资源 $k$ 已经饱和， 我们有  $t_{i,k} \geq \frac 1 n $，进一步的，根据主导份额的定义，我们有 $ s_i \geq t_{i,k} \geq \frac 1 n  $，因为渐进式填充算法以相同的速率提高每个用户的主导份额，每个用户至少可以获得她的主导资源的 $\frac 1 n$，因此，DRF算法满足“共享激励”属性。如果所有用户的主导资源都相同，那么每个用户都可以获得该资源的 $\frac 1 n$，所以，DRF 算法也满足“瓶颈公平性”属性。
#### 定理11 每一个 DRF 分配都是“无嫉妒”的

#### 证明
举一个反证，假设用户 $i$ 嫉妒另外一个用户 $j$。如果用户 $i$ 嫉妒用户 $j$，那么用户 $j$ 的每一个资源的份额都比用户 $i$ 需要的要高，否则的话用户 $i$ 就不能在用户 $j$ 的分配下运行更多的任务。这意味着用户 $j$ 的主导份额比用户 $i$ 的主导份额要大。因为每一种资源在分配给 $i$ 的同时都会分配给 $j$，所以用户 $j$ 不可能在用户 $i$ 的后面达到资源饱和，例如， $t_j \leq t_i$，$t_k$是用户 $k$ 由于资源饱和从而冻结分配的时间。但是，如果 $t_j \leq t_i$ 的话，在渐进式填充算法中，用户 $j$ 和用户 $i$ 将会在时间 $t_j$ 这一刻相等，在此之后，用户 $i$ 的主导份额只能增加，这与假设相违背。

#### 定理12 （策略防伪）在DRF算法里，用户不能通过更改其需求向量来提高主导份额

#### 证明
假设用户 $i$ 可以通过使用一个需求向量 $ \hat{d}_i \neq d_i $ 提高她的主导份额。$a_{i,j}$ 和 $\hat{a}_{i,j}$ 分别代表资源 $j$ 对于用户 $i$
的需求向量  $ {d}_i$ 和 $d_i$ 的分配数量。如果用户 $i$ 使用向量 $\hat{d}_i$ 可以得到更好的分配，那么，对于每一个资源 $k$ $(d_{i,k} > 0) $ ，都有 $ \hat{a}_{i,j} \geq a_{i,j}$，设 $r$ 是用户 $i$ 使用需求向量 $d_i$ 的第一个饱和资源。如果其他的用户都没有获得资源 $r$ (对于所有的 $j \neq i, a_{j,r} = 0$)，这将与假设相冲突，用户 $i$ 已经获得了全部的资源 $r$，她无法通过新的需求向量  $\hat{d}_i$ 来提高其资源 $r$ 的分配。假设其他的用户也分配得到了资源 $r$，(对于一些 $j \neq i, a_{j,r} \geq 0$)，在这种情况下，使用 $d_i$ 的渐进式填充算法最终将在时刻 $t$ 用尽资源 $r$，使用 $\hat{d}_i$ 将在时刻 $t'$ 用尽。回忆一下，主导份额是一个用户的份额的最大值，因此 $i$ 在 $\hat{a}$ 时必须比 $a$ 有更高的主导份额。因为渐进式填充总是以一个固定的速率增加主导份额，因此, $t' > t$。这暗示了当 $i$ 使用 $\hat{d}_i$，在时刻 $t'$ 之前没有用光任何的资源，因此在时刻 $t'$ 之前并不影响其他的用户分配。当 $i$ 使用 $\hat{d}_i$，任意一个使用资源 $r$的用户 $m$ 在时刻 $t$ 得到的分配是 $a_{i,r}$ 。因此，在时刻 $t$, 资源 $r$ 只有 $a_{i,r}$ 可提供给用户 $i$，这与假设 $ \hat{a}_{i,j} > a_{i,j}$ 相冲突。

DRF算法的“策略防伪”属性显示，一个用户不会因为她要的多就会得到更好的分配。下面这个例子表明，过多的需求事实上会降低该用户的分配，从而导致主导份额较低。假设一个集群里有两种资源和10个用户，第一个用户的需求向量是 <1, 0>，其他用户的需求向量都是 <0, 1>。第一个用户获得了第一种资源的全部，其他用户每人都获得了第二种资源的 $ \frac 1 9$。如果第一个用户将其需求向量变更为 <1, 1>，她将只能分得每种资源得 $\frac 1 {10} $，其他用户分别得到第二种资源的 $\frac 1 {10} $。

在实践中，由于数据中心的资源通常分布在不同的物理机上，从而导致资源碎片化，使得这种情况可变得更糟糕。人为地增加一个人的需求可能会导致这样一种情况：虽然总的资源
是够的，但是在任何一台机器上都不够满足新的需求。有关更多信息，请参阅第 6.2 节。

接下来，为简单起见，我们假设需求向量都是严格为正的，即每个用户对每个资源的需求都不为零。

#### 定理13 给定严格的正需求向量，DRF 算法保证每个用户的主导份额是相等的。即：对于所有的用户 $i$ 和 $j$，每一次DRF分配，保证 $s_i=s_j$

#### 证明
渐进式填充算法从一开始就以相同的速率提高每一个用户的主导份额，直到一种资源达到饱和。此时，任何用户都无法得到更多的资源因为每个用户都要求得到一个饱和资源的正值。

#### 定理14 给定严格的正需求，DRF 满足总体单调性

#### 证明
考虑任何 DRF 分配。非零需求意味着所有用户都有相同的饱和资源。考虑删除一个用户并放弃其资源（每种资源都有一定数量），由于所有用户都有相同的主导份额 $\alpha$, 因为 "Pareto效率"，任何将任意用户 $i$ 的主导份额降低到 $\alpha$ 以下的新分配将使得另一个用户$j$ 的主导份额超过 $\alpha$，这将违反最大-最小公平性原则。因为这有可能通过减少 $j$ 的分配来增加 $i$ 的主导份额，而此时$j$的主导份额是大于 $i$ 的。

然而，我们注意到，在没有严格的正需求向量的情况下，DRF 不再满足总体单调性【14】。
