...
图8(a)和8(b)展示了某个任务的CPU 和内存的分配对时间的函数， 图8(c)展示了任务的主导份额对事件的函数。在前两分钟，工作1每个任务使用了<1 CPU, 10 GB 内存>，工作2每个任务使用<1 CPU, 1 GB 内存>。工作1的主导份额是内存，工作2的主导份额是CPU。注意 DRF 使两个工作的主导份额相等。
